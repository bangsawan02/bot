...
class DownloaderBot:
    def __init__(self, url):
        ...
        # 1. Selector CSS valid untuk wait_for_selector (Tanpa "text=")
        self.wait_query = "button, a, form[name='F1'], .downloadbtn, #downloadButton, [role='button']"
        
        # 2. Selector detail untuk identifikasi (Boleh pakai text/regex Playwright)
    ...
    async def _generic_browser_handler(self, page):
        ...
        try:
            ...
            if target_el:
                await self._notify("🎯 Tombol ditemukan! Mengeksekusi...")
                ...
                raise PlaywrightTimeout("Selector ada tapi tidak visible/cocok.")

        except PlaywrightTimeout:
            await self._notify("⏰ Timeout 10s: Tombol tidak ditemukan.")
            shot_path = "timeout_error.png"
            await page.screenshot(path=shot_path)
            await self._send_screenshot(shot_path, f"❌ Timeout 10s di: {self.url}")
            return None
    ...
    async def run(self):
        ...
        async with Stealth().use_async(async_playwright()) as p:
            ...
            try:
                ...
                # Langsung gas, nunggu dom ready aja biar cepet
                await page.goto(self.url, wait_until="domcontentloaded", timeout=30000)
                
                # Kasih jeda dikit buat Gofile render list file
                await asyncio.sleep(2)

                result = await self._generic_browser_handler(page)
                if result:
                    await self._notify(f"✅ BERHASIL: {result}")
                    # Hapus screenshot lama kalau ada
